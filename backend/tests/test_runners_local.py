...
def helper_test_runner(runner, filename, code):
    ...
    assert (
        result["exit_code"] == 0
    ), f"{runner.language_name} failed with exit code {result['exit_code']}. Output: {result['output']}"
    assert (
        "Hello World" in result["output"]
    ), f"{runner.language_name} output missing Hello World. Output: {result['output']}"
