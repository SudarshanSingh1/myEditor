...
def project_id(client: TestClient, workspace_user_token: str) -> str:
    ...
    response = client.post(
        "/api/v1/projects",
        cookies=cookies,
        json={"name": "Workspace Test Project", "visibility": "PRIVATE"},
    )
    return response.json()["data"]["id"]


def test_create_and_get_folder(
    client: TestClient, workspace_user_token: str, project_id: str
):
    cookies = {"access_token": workspace_user_token}
    ...
    # Check tree
    tree_res = client.get(
        f"/api/v1/workspace/projects/{project_id}/tree", cookies=cookies
    )
...
def test_duplicate_and_delete_file(
    client: TestClient, workspace_user_token: str, project_id: str
):
    cookies = {"access_token": workspace_user_token}
    ...
    f_res = client.post(
        "/api/v1/workspace/files",
        cookies=cookies,
        json={"name": "utils.ts", "project_id": project_id},
    )
    ...
    # Duplicate
    dup_res = client.post(
        f"/api/v1/workspace/files/{file_id}/duplicate", cookies=cookies
    )
...
def test_rename_folder_and_cascade_soft_delete(
    client: TestClient, workspace_user_token: str, project_id: str
):
    cookies = {"access_token": workspace_user_token}
