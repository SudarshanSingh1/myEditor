...
def admin_token(client: TestClient, db_session) -> str:
    ...
    user = (
        ...
        .filter(User.email == "adminuser_test@example.com")
        .first()
    )
    ...
    login_resp = client.post(
        "/api/v1/auth/login",
        json={"email": "adminuser_test@example.com", "password": "Password123!"},
    )
...
def test_admin_projects_and_details(client: TestClient, admin_token: str, db_session):
    ...
    details_resp = client.get(
        f"/api/v1/admin/projects/{proj.id}/details", cookies=cookies
    )
    assert (
        details_resp.status_code == 200
    ), f"Failed on GET /admin/projects/{proj.id}/details: {details_resp.text}"
...
def test_admin_users(client: TestClient, admin_token: str, db_session):
    ...
    assert (
        details_resp.status_code == 200
    ), f"Failed on GET /admin/users/details: {details_resp.text}"
