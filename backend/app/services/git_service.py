...
class GitService:
    ...
    def _get_authenticated_url(self, repo_url: str, access_token: str) -> str:
        ...
        if repo_url.startswith("https://"):
            return repo_url.replace("https://", f"https://x-access-token:{access_token}@")
        return repo_url

    def clone_repository(self, project: Project, repo_url: str, access_token: str, branch: Optional[str] = None):
            ...
            try:
                ...
                for root, dirs, files in os.walk(tmpdir):
                    ...
                    for d in dirs:
                        ...
                        new_folder = WorkspaceRepository.create_folder(self.db, folder_in, path="", depth=0)
    ...
    def push_to_repository(self, project: Project, repo_url: str, access_token: str, commit_message: str, branch: str = "main"):
        """
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                all_folders = self.db.query(Folder).filter(Folder.project_id == project.id, Folder.deleted_at.is_(None)).all()
                folder_paths = {f.id: f.path for f in all_folders}
                
                # Fetch all files with content
                all_files = self.db.query(File).filter(File.project_id == project.id, File.deleted_at.is_(None)).all()
                ...
                user_email = project.owner.email if project.owner else "editor@hamara.com"
                user_name = project.owner.first_name if project.owner else "Hamara Editor"
                ...
                return {"status": "success", "message": f"Successfully pushed to {branch}"}
