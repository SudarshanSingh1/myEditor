...
class ProjectService:
    ...
    def _get_project_and_check_owner(
        self, project_id: UUID, user_id: UUID, allow_deleted: bool = False
    ) -> Project:
            ...
            for name, content in current_level.items():
                if isinstance(content, dict):
                    ...
                    folder_in = FolderCreate(
                        project_id=project.id, name=name, parent_id=parent_id
                    )
                    try:
                        folder = WorkspaceService.create_folder(
                            self.db, folder_in, user
                        )
                        _create_tree(content, folder.id)
                    except Exception as e:
                        logger.error(
                            f"Error creating folder {name}: {e}", exc_info=True
                        )
                else:
                    # It's a file
                    file_in = FileCreate(
    ...
    def restore_project(self, project_id: UUID, user_id: UUID) -> ProjectResponse:
        project = self._get_project_and_check_owner(
            project_id, user_id, allow_deleted=True
        )
        if not project.deleted_at:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Project is not in trash",
            )
    ...
    def hard_delete_project(self, project_id: UUID, user_id: UUID) -> None:
        ...
        if not project.deleted_at:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Project must be soft-deleted first",
            )

        self.repo.hard_delete(project)

    def empty_trash(self, user_id: UUID) -> None:
        trashed_projects, _ = self.repo.get_user_projects(
            user_id, limit=1000, deleted=True
        )
