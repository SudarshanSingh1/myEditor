...
class AuthService:
    """
    Service layer for Authentication, Authorization, and Session Management.
    
    This service handles all critical security operations including password validation,
    JWT generation, TOTP 2FA verification, refresh token rotation with replay detection,
    and audit logging for all authentication events.
    """
    @staticmethod
    def register_user(
        db: Session, req: UserRegisterRequest, ip_address: str = None
    ) -> User:
        ...
        try:
            ...
            logger.error(
                f"Failed to send verification email to {email_normalized}: {e}"
            )
    ...
    def resend_verification(db: Session, email: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to resend verification email to {email_normalized}: {e}"
            )
            raise HTTPException(
    ...
    def authenticate_user(
        ...
        if maint_config.get("enabled"):
            ...
            if user.role == RoleEnum.OWNER:
                ...
                if user.role in (RoleEnum.ADMIN, RoleEnum.MODERATOR):
                    ...
                    "system.maintenance.bypass" in user.effective_permissions
                    or "*" in user.effective_permissions
                ):
                    is_allowed = True
            if not is_allowed:
                logger.warning(
                    f"Login failed: User {user.username} blocked by maintenance mode."
                )
                raise HTTPException(
        ...
        logger.debug(
            f"User found: ID {user.id}, Username {user.username}. Verifying password hash..."
        )
        is_valid_password = verify_password(req.password, user.password_hash)
        logger.debug(
            f"Password verification result: {'Success' if is_valid_password else 'Failed'}"
        )
        ...
        if (
            ...
            and user.temp_password_expires_at
        ...
        browser = (
            _ua_clean(ua.browser.family if hasattr(ua, "browser") else "") or "Unknown"
        )
    ...
    def complete_2fa_login(
        ...
        try:
            payload = jwt.decode(
                pre_auth_token, settings.SECRET_KEY, algorithms=[settings.JWT_ALGORITHM]
            )
        except JWTError:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    @staticmethod
    def change_password(
        db: Session, user: User, req: ChangePasswordRequest, ip_address: str = None
    ):
    ...
    @staticmethod
    def update_profile(
        db: Session, user: User, req: UserProfileUpdateRequest, ip_address: str = None
    ) -> User:
    ...
    def request_password_reset(
        db: Session, req: PasswordResetRequest, ip_address: str = None
    ) -> str:
        ...
        # Log action
        log = AuditLog(
            user_id=user.id, action="REQUEST_PASSWORD_RESET", ip_address=ip_address
        )
        db.add(log)
        db.commit()
    ...
    def reset_password_confirm(
        db: Session, req: ResetPasswordConfirmRequest, ip_address: str = None
    ):
        try:
            payload = jwt.decode(
