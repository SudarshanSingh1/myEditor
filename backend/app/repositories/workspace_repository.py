...
class WorkspaceRepository:
    ...
    def get_folder(db: Session, folder_id: UUID) -> Optional[Folder]:
        return db.execute(select(Folder).where(Folder.id == folder_id, Folder.deleted_at.is_(None))).scalar_one_or_none()

    @staticmethod
    def get_folder_by_name(db: Session, project_id: UUID, name: str, parent_id: Optional[UUID] = None) -> Optional[Folder]:
    ...
    @staticmethod
    def create_folder(db: Session, obj_in: FolderCreate, path: str, depth: int) -> Folder:
        db_obj = Folder(
    ...
    @staticmethod
    def update_folder(db: Session, db_obj: Folder, obj_in: FolderUpdate, new_path: Optional[str] = None, new_depth: Optional[int] = None) -> Folder:
    ...
    def _get_all_descendants_ids(db: Session, parent_id: UUID) -> List[UUID]:
        ids = []
        children = db.execute(select(Folder.id).where(Folder.parent_id == parent_id, Folder.deleted_at.is_(None))).scalars().all()
    ...
    # ---------------------------------------------------------
    @staticmethod
    def get_file(db: Session, file_id: UUID, for_update: bool = False) -> Optional[File]:
    ...
    @staticmethod
    def get_file_by_name(db: Session, project_id: UUID, name: str, folder_id: Optional[UUID] = None) -> Optional[File]:
    ...
    @staticmethod
    def update_file(db: Session, db_obj: File, obj_in: FileUpdate, extension: Optional[str] = None) -> File:
        update_data = obj_in.model_dump(exclude_unset=True)
    ...
    # ---------------------------------------------------------
    @staticmethod
    def create_file_version(db: Session, file: File, current_user_id: Optional[UUID] = None) -> FileVersion:
        version = FileVersion(
            file_id=file.id,
    ...
    @staticmethod
    def get_file_version(db: Session, file_id: UUID, version_number: int) -> Optional[FileVersion]:
        return db.execute(
            select(FileVersion)
            .where(FileVersion.file_id == file_id, FileVersion.version_number == version_number)
        ).scalar_one_or_none()
