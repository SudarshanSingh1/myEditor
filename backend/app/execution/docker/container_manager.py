...
from app.execution.docker.container_security import (
    get_secure_container_config,
    MAX_EXECUTION_TIME,
    MAX_OUTPUT_SIZE,
)
...
class DockerManager:
    ...
    def get_client(cls):
        if cls._client is None:
            try:
                ...
                raise RuntimeError(
                    "Docker daemon is not available. Execution engine cannot start."
                )
        return cls._client
    ...
    def run_container(
        ...
        config = get_secure_container_config(
            image, command, working_dir, binds, mem_limit
        )
        ...
        try:
            ...
            try:
                ...
                return (
                    -1,
                    f"Execution Error: Docker API failure ({type(e).__name__})",
                ...
                return (
                    124,
                    "Execution Error: Time Limit Exceeded",
                    time.time() - start_time,
                )
            ...
            logger.error(
                f"Docker daemon error during execution (resetting client): {e}"
            )
            cls._client = None
            return (
                -1,
                "Execution Error: Docker daemon unavailable. Please try again.",
                time.time() - start_time,
            )
            ...
            return (
                -1,
                "Internal System Error. Check server logs for details.",
                time.time() - start_time,
            )
        finally:
            # Ensure container is ALWAYS destroyed without blocking the API response
            if container:
                import threading
                ...
                def _remove():
                    try:
                        ...
                        logger.error(
                            f"Failed to remove container {container.id}: {remove_err}"
                        )

                threading.Thread(target=_remove, daemon=True).start()

    @classmethod
    async def run_container_interactive(
        cls,
        image: str,
        command: str,
        working_dir: str,
        ...
        try:
                ...
                while True:
                    try:
                        ...
                        try:
                            ...
                            # Backward compatibility: if it's not JSON, assume raw stdin
            ...
            wait_task = asyncio.create_task(
                asyncio.to_thread(container.wait, timeout=interactive_timeout)
            )
