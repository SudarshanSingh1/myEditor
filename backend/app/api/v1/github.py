...
def get_github_token(user: User) -> str:
    oauth_acc = next((acc for acc in user.oauth_accounts if acc.provider == "github"), None)
...
async def create_repository(
    ...
    service = GitHubService(token)
    try:
        repo = await service.create_repository(name=name, description=description, private=private)
