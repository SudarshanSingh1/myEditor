...
# -------------------------------------------------------------------
@router.get(
    "/projects/{project_id}/tree", response_model=SuccessResponse[WorkspaceTreeResponse]
)
...
def save_file(
    ...
    file = WorkspaceService.save_file(
        db, req.id, req.expected_version, req.content, current_user
    )
...
@router.get(
    ...
    response_model=SuccessResponse[FileVersionWithContentResponse],
)
def get_file_version(
    ...
    version = WorkspaceService.get_file_version(
        db, file_id, version_number, current_user
    )
    return SuccessResponse(message="File version retrieved.", data=version)


@router.post(
    "/files/{file_id}/restore/{version_number}",
    response_model=SuccessResponse[FileResponse],
)
def restore_file_version(
    ...
    return SuccessResponse(
        message=f"File restored to version {version_number}.", data=file
    )


@router.delete(
    "/files/{file_id}/versions/{version_number}", response_model=SuccessResponse
)
...
def migrate_guest_workspace(
    req: GuestMigrationRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    # Create new project for the migrated files
    new_project = Project(
        name=req.project_name, owner_id=current_user.id, is_public=False
    )
    db.add(new_project)
    ...
    return SuccessResponse(
        message="Guest workspace migrated successfully.",
        data={"project_id": str(new_project.id)},
    )
...
def search_project_files(
    ...
    use_regex: bool = Query(
        False, alias="regex", description="Treat q as a regular expression"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
