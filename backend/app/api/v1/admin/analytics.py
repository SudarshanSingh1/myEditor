...
def get_daily_active_users(
    ...
    rows = (
        ...
        .filter(User.last_login >= since, User.is_deleted == False)
...
def get_execution_trends(
    ...
    rows = (
        ...
        .filter(ExecutionLog.created_at >= since)
...
def get_master_timeline(
    ...
    errors = (
        db.query(
            func.date(SystemError.created_at).label("date"),
            func.count(SystemError.id).label("count"),
        )
        .filter(SystemError.created_at >= since)
        .group_by(func.date(SystemError.created_at))
        .all()
    )
    ...
    for r in users:
        ...
        if d not in timeline_dict:
            timeline_dict[d] = {"date": d, "users": 0, "executions": 0, "errors": 0}
    ...
    return SuccessResponse(
        message="Master timeline retrieved",
        data={
            "items": sorted_items,
...
def get_language_distribution(
    ...
    rows = (
        db.query(Project.language, func.count(Project.id).label("count"))
    ...
    return SuccessResponse(
        message="Language distribution retrieved", data={"items": data}
    )
...
def get_execution_status(
    ...
    rows = (
        db.query(ExecutionLog.status, func.count(ExecutionLog.id).label("count"))
...
def get_feedback_resolution_timeline(
    db: Session = Depends(get_db),
    admin: User = Depends(require_permission("users.read.basic")),
):
    ...
    rows = (
        ...
        .group_by(func.date(Feedback.created_at), Feedback.status)
        .all()
    )
    ...
    sorted_items = sorted(list(timeline_dict.values()), key=lambda x: x["date"])
    return SuccessResponse(
        message="Feedback resolution timeline retrieved", data={"items": sorted_items}
    )
