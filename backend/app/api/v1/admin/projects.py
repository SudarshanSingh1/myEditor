...
def get_projects(
    ...
    file_counts = (
        dict(
            db.query(File.project_id, func.count(File.id))
    ...
    exec_counts = (
        dict(
            db.query(ExecutionLog.project_id, func.count(ExecutionLog.id))
            .filter(ExecutionLog.project_id.in_(project_ids))
            .group_by(ExecutionLog.project_id)
            .all()
        )
        if project_ids
        else {}
    ...
    return SuccessResponse(
        message="Projects retrieved", data={"items": projects_list, "total": total}
    )
...
def get_project_details(
    ...
    execs = (
        ...
        .filter(ExecutionLog.project_id == proj.id)
    ...
    file_data = [
        {"name": f.name, "size": f.size, "language": f.language} for f in files
    ]
...
def perform_project_action(
    ...
    if req.action == "archive":
        ...
        for folder in folders:
            new_folder = Folder(
                project_id=new_proj.id, name=folder.name, parent_id=None
            )
            db.add(new_folder)
            db.flush()
        ...
        for folder in folders:
            if folder.parent_id:
                new_f = (
                    db.query(Folder).filter(Folder.id == folder_map[folder.id]).first()
                )
    ...
    AuditService.log_action(
        db,
        admin.id,
        f"PROJECT_ACTION_{action_log}",
        request.client.host,
        request.headers.get("user-agent"),
        {"project_id": str(project_id), "reason": req.reason},
    )
...
def perform_bulk_project_action(
    ...
    projects = (
        db.query(Project)
        .options(joinedload(Project.owner))
        .filter(Project.id.in_(req.project_ids))
        .all()
    )
    if not projects:
    ...
    for p in projects:
        if req.action == "archive":
            ...
            file_ids = [
                f.id for f in db.query(File.id).filter(File.project_id == p.id).all()
            ]
            if file_ids:
    ...
    AuditService.log_action(
        db,
        admin.id,
        f"BULK_PROJECT_ACTION_{action_log}",
        request.client.host,
        request.headers.get("user-agent"),
        {"target_count": modified_count, "action": req.action},
    )

    return SuccessResponse(
        message=f"Bulk action '{req.action}' performed on {modified_count} projects"
    )
