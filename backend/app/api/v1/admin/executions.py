...
def get_executions_dashboard(
    ...
    row = db.query(
        ...
        func.count(
            sa_case(
                (cast(ExecutionLog.status, String) == ExecutionStatus.RUNNING.value, 1)
            )
        ).label("running"),
        func.count(
            sa_case(
                (cast(ExecutionLog.status, String) == ExecutionStatus.QUEUED.value, 1)
            )
        ).label("queued"),
        func.count(
            sa_case(
                (cast(ExecutionLog.status, String) == ExecutionStatus.SUCCESS.value, 1)
            )
        ).label("completed"),
        func.count(
            sa_case(
                (
                    cast(ExecutionLog.status, String)
        ...
        func.count(
            sa_case(
                (
                    cast(ExecutionLog.status, String).in_(
                        [
...
def get_executions(
    ...
    query = (
        db.query(ExecutionLog, User, Project)
        .outerjoin(User, ExecutionLog.user_id == User.id)
        .outerjoin(Project, ExecutionLog.project_id == Project.id)
    )

    if search:
        query = query.filter(
            User.username.ilike(f"%{search}%") | Project.name.ilike(f"%{search}%")
        )
    if status:
    ...
    return SuccessResponse(
        message="Executions retrieved", data={"items": items, "total": total}
    )
...
def get_executions_audit(
    ...
    query = (
        ...
        .filter(AuditLog.action.like("EXECUTION_%"))
    )
    ...
    return SuccessResponse(
        message="Execution audits retrieved", data={"items": items, "total": total}
    )
...
def get_execution_details(
    ...
    row = (
        ...
        .outerjoin(User, ExecutionLog.user_id == User.id)
        .outerjoin(Project, ExecutionLog.project_id == Project.id)
        .filter(ExecutionLog.id == execution_id)
        .first()
    )
    if not row:
...
def download_execution_logs(
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="execution_{execution_id}.log"'
        },
    )
