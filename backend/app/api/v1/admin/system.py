...
def get_system_settings(
    ...
    if getattr(settings_row, "maintenance_mode", False) and getattr(
    ...
    data = {
        ...
        "maintenance_end_time": settings_row.maintenance_end_time.isoformat()
        if getattr(settings_row, "maintenance_end_time", None)
        else None,
        "maintenance_allow_admin_access": getattr(
            settings_row, "maintenance_allow_admin_access", True
        ),
        "maintenance_show_countdown": getattr(
            settings_row, "maintenance_show_countdown", True
        ),
        ...
        "max_execution_time_seconds": getattr(
            settings_row, "max_execution_time_seconds", 30
        ),
        ...
        if getattr(settings_row, "oauth_google_client_secret", None)
        else None,
        ...
        if getattr(settings_row, "oauth_github_client_secret", None)
        else None,
...
def update_system_settings(
    ...
    if should_validate_smtp:
        ...
        test_host = (
            ...
            else getattr(settings_row, "smtp_host", None)
        )
        test_port = (
            req.smtp_port
            if req.smtp_port is not None
            else getattr(settings_row, "smtp_port", 587)
        )
        test_user = (
            req.smtp_user
            if req.smtp_user is not None
            else getattr(settings_row, "smtp_user", None)
        )
        ...
        test_tls = (
            ...
            else getattr(settings_row, "smtp_tls", True)
        )
        test_ssl = (
            req.smtp_ssl
            if req.smtp_ssl is not None
            else getattr(settings_row, "smtp_ssl", False)
        )
        ...
        if test_host:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"SMTP Validation Failed: {str(e)}"
                )
...
def get_audit_logs(
    ...
    return SuccessResponse(
        message="Audit logs retrieved", data={"items": log_list, "total": total}
    )
...
def export_audit_logs(
    ...
    if format == "json":
        ...
        def _csv_row_generator():
            ...
            w = csv.writer(header)
            w.writerow(
                ["ID", "Timestamp", "Action", "Username", "IP Address", "Details"]
            )
        ...
        data.append(
            {
                ...
                "username": un or "Anonymous",
                "priority": f.priority,
                "description": getattr(f, "description", ""),
            }
        )
    return SuccessResponse(
        message="Feedback retrieved", data={"items": data, "total": total}
    )
...
def get_errors(
    ...
    return SuccessResponse(
        message="Errors retrieved", data={"items": items, "total": total}
    )
...
def bulk_delete_system_errors(
    ...
    try:
        deleted_count = (
            db.query(SystemError)
            .filter(SystemError.id.in_(req.ids))
        ...
        AuditService.log_action(
            db,
            admin.id,
            "BULK_DELETE_SYSTEM_ERRORS",
            request.client.host,
            request.headers.get("user-agent"),
            {"deleted_count": deleted_count, "requested_count": len(req.ids)},
        )
        return SuccessResponse(
            message=f"Deleted {deleted_count} errors successfully",
...
def get_email_logs(
        ...
        items.append(
            {
                ...
                "subject": log.subject,
                "user_role": getattr(log.user_role, "value", log.user_role)
                if log.user_role
                else None,
    ...
    return SuccessResponse(
        message="Email logs retrieved", data={"items": items, "total": total}
    )
