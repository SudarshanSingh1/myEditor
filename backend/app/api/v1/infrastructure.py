...
def get_backups(
    ...
    return {
        "success": True,
        "data": [
            {"id": b.id, "filename": b.filename, "status": b.status, "type": b.type}
            for b in backups
        ],
    }
...
def create_backup(
    ...
    AuditService.log_action(
        db, current_user.id, "TRIGGER_BACKUP", details={"backup_id": new_backup.id}
    )
    return {
        "success": True,
        "message": "Backup triggered successfully",
        "data": {"id": new_backup.id, "filename": new_backup.filename},
    }
...
def create_deployment(
    ...
    db.refresh(new_dep)
    AuditService.log_action(
        db, current_user.id, "RECORD_DEPLOYMENT", details={"deployment_id": new_dep.id}
    )
    return {
        "success": True,
        "message": "Deployment recorded",
        "data": {"id": new_dep.id, "version": new_dep.version},
    }
...
def factory_reset(
    ...
    try:
        ...
        AuditService.log_action(
            db, current_user.id, "FACTORY_RESET", details={"scope": cleared}
        )
        return {
            "success": True,
            "message": f"Factory reset completed. Cleared: {', '.join(cleared)}",
        }
