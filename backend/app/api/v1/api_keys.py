...
def create_api_key(
    ...
    new_key = ApiKey(name=name, key_hash=key_hash, prefix=prefix, scopes=["*"])
    db.add(new_key)
    db.commit()
    ...
    return {
        ...
        "data": {"raw_key": raw_key, "id": new_key.id, "prefix": new_key.prefix},
    }


@router.post(
    "/{key_id}/revoke",
    dependencies=[Depends(require_permission("system.settings.edit"))],
)
def revoke_api_key(
    key_id: str,
    db: Session = Depends(get_db),
