...
def submit_feedback(req: FeedbackCreate, request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_dep)):
...
def update_feedback_status(feedback_id: uuid.UUID, req: FeedbackUpdateStatus, request: Request, db: Session = Depends(get_db), current_admin: User = Depends(get_current_admin_dep)):
    ...
    AuditService.log_action(db, current_admin.id, "UPDATE_FEEDBACK_STATUS", request.client.host, request.headers.get("user-agent"), {"feedback_id": str(feedback_id), "old_status": old_status, "new_status": req.status})
...
def delete_feedback(feedback_id: uuid.UUID, request: Request, db: Session = Depends(get_db), current_admin: User = Depends(get_current_admin_dep)):
    item = db.query(Feedback).filter(Feedback.id == feedback_id).first()
    if not item:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Feedback not found")
    ...
    AuditService.log_action(db, current_admin.id, "DELETE_FEEDBACK", request.client.host, request.headers.get("user-agent"), {"feedback_id": str(feedback_id)})
