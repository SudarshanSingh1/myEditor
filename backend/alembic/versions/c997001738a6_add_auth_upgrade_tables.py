...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_user_sessions_session_token_jti"),
        "user_sessions",
        ["session_token_jti"],
        unique=True,
    )
    ...
    op.add_column(
        "users", sa.Column("recovery_codes", sa.String(length=1000), nullable=True)
    )
    op.add_column(
        "users",
        sa.Column(
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_user_sessions_session_token_jti"), table_name="user_sessions"
    )
