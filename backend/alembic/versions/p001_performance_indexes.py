...
def upgrade() -> None:
    _exec("CREATE INDEX IF NOT EXISTS ix_user_sessions_user_id_active ON user_sessions (user_id, is_active) WHERE is_active = true")
    _exec("CREATE INDEX IF NOT EXISTS ix_audit_logs_created_at_user_id ON audit_logs (created_at DESC, user_id)")
    ...
    _exec("CREATE INDEX IF NOT EXISTS ix_user_activities_user_id_date ON user_activities (user_id, activity_date DESC)")
