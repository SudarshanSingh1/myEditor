...
export default function ProjectWorkspace({ projectId }: { projectId?: string } = {}) {
  ...
  const { status: githubStatus, fetchStatus: fetchGitHubStatus } = useGitHubStore();
      ...
      <div className="w-12 h-full border-r bg-[#1e1e1e] border-[#333] flex flex-col justify-between py-2 z-50 flex-shrink-0 text-gray-400 select-none">
        ...
        <div className="flex flex-col items-center gap-1 mb-2">
          <Dropdown
            ...
            trigger={
              <button
                className="p-3 transition-colors hover:text-white w-full flex justify-center relative"
                title="Accounts"
                onClick={() => fetchGitHubStatus()}
              >
                <CircleUser className="w-6 h-6 stroke-[1.5px]" />
                {githubStatus?.connected && (
                  <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-green-500 ring-1 ring-black" />
                )}
              </button>
            }
          >
            ...
            <div className="px-2 py-1.5">
              <div className="text-xs text-muted-foreground mb-1.5 font-medium">GitHub</div>
              {githubStatus?.connected ? (
                <div className="space-y-1.5">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        const { githubApi } = await import('../../lib/api/github');
                ...
                <button
                  className="flex items-center gap-2 text-xs text-foreground hover:text-white transition-colors"
