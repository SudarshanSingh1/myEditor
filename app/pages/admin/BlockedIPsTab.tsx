  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center border-b border-black/10 dark:border-white/10 pb-4">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Blocked IPs</h2>
        <button
          onClick={() => setShowBlock(true)}
          className="px-4 py-2 bg-red-600 hover:bg-red-700 text-gray-900 dark:text-white text-sm font-medium rounded-lg transition-colors"
        >
          + Block IP
        </button>
      </div>
      ...
      {showBlock && (
        <div className="p-4 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl space-y-4">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white">Block New IP</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <input
              type="text" placeholder="IP Address" value={form.ip_address}
              onChange={e => setForm(f => ({ ...f, ip_address: e.target.value }))}
              className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white"
            />
            <input
          ...
          <div className="flex gap-2">
            <button onClick={handleBlock} className="px-4 py-2 bg-red-600 text-gray-900 dark:text-white text-sm rounded-lg">Confirm Block</button>
            <button onClick={() => setShowBlock(false)} className="px-4 py-2 bg-black/10 dark:bg-white/10 text-gray-900 dark:text-white text-sm rounded-lg">Cancel</button>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr key={ip.id} className="hover:bg-black/5 dark:bg-white/5 transition-colors">
                  <td className="px-4 py-3 font-mono text-gray-900 dark:text-white font-medium">{ip.ip_address}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{ip.reason || "—"}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Dropdown
                      align="right"
                      trigger={<button className="p-1.5 text-gray-500 hover:text-gray-900 dark:text-white rounded hover:bg-black/10 dark:bg-white/10"><MoreHorizontal className="w-4 h-4" /></button>}
                    >
