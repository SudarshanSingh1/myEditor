    ...
    <div style={{ background: "var(--e-bg-base)", minHeight: "100%" }}>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <input type="text" placeholder="Search projects..." value={search}
          ...
          className="w-full sm:w-auto flex-1 max-w-sm bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-600 focus:outline-none focus:border-violet-500/50"
        />
        <select value={language} onChange={e => { setLanguage(e.target.value); setPage(0); }}
          className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-violet-500/50">
        ...
        <button onClick={async () => { await fetchProjects(); toast.success("Projects refreshed"); }} className="px-4 py-2 bg-white/8 hover:bg-white/12 text-sm text-gray-700 dark:text-gray-300 rounded-lg border border-black/10 dark:border-white/10">Refresh</button>
      </div>
      ...
      <div className="rounded-xl border border-black/10 dark:border-white/8 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[800px]">
            <thead>
              <tr className="bg-black/5 dark:bg-white/3 border-b border-black/10 dark:border-white/8">
                <th className="px-4 py-3 text-left w-10">
                  <input type="checkbox" className="rounded border-white/20 bg-black/5 dark:bg-white/5 checked:bg-violet-500 focus:ring-violet-500 focus:ring-offset-gray-900" 
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr key={p.id} className={`hover:bg-black/5 dark:bg-white/3 transition-colors ${selectedIds.has(p.id) ? 'bg-violet-500/5' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{p.owner_username}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => setSelectedProjectId(p.id)}
                      className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-black/5 dark:bg-white/5 text-gray-700 dark:text-gray-300 hover:bg-black/10 dark:bg-white/10 transition-colors border border-black/10 dark:border-white/10">
        ...
        {total > limit && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-black/10 dark:border-white/8">
            <p className="text-xs text-gray-500">Showing {page * limit + 1}–{Math.min((page + 1) * limit, total)} of {total}</p>
            <div className="flex gap-2">
              <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="px-3 py-1 text-xs rounded-lg border border-black/10 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:bg-black/5 dark:bg-white/5 disabled:opacity-40">Previous</button>
              <button disabled={(page + 1) * limit >= total} onClick={() => setPage(p => p + 1)} className="px-3 py-1 text-xs rounded-lg border border-black/10 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:bg-black/5 dark:bg-white/5 disabled:opacity-40">Next</button>
            </div>
          </div>
        )}
      </div>
