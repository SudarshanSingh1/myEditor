...
const EditorActions: React.FC = React.memo(() => {
  ...
  const resolvedProjectId = routeProjectId || (projectLanguage ? `guest-${projectLanguage}-project` : null);
    ...
    <div className="flex items-center px-2 space-x-2 border-l border-border bg-background">
      ...
      <button
        ...
        onClick={() => {
          ...
          if (resolvedProjectId && activeFileId && displayLanguage) {
            window.dispatchEvent(new CustomEvent('IDE_OPEN_PANEL', { detail: { tab: 'TERMINAL' } }));
            runCode(resolvedProjectId, activeFileId, displayLanguage);
          }
        }}
        className={cn(
